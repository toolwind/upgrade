...
const debugLog = (inDebugMode: boolean, ...args: any[]) => {
  if (inDebugMode) {
...
export function args<const T extends Arg>(options: T, argv = process.argv.slice(2)): Result<T> {
  ...
  for (let [
    ...
    debugLog(inDebugMode, `Initializing ${flag} with default:`, JSON.stringify(defaultValue))

    if (alias) {
      let key = alias.slice(1)
      if (parsed[key] !== undefined) {
        const convertedValue = convert(parsed[key], type, inDebugMode)
        debugLog(
          inDebugMode,
    ...
    debugLog(inDebugMode, `Final value for ${flag} after checks:`, JSON.stringify(result[flag]))
  }

  debugLog(inDebugMode, 'Returning final args object:', JSON.stringify(result))
...
function convert<T extends keyof Types>(value: ArgumentType, type: T, inDebugMode: boolean): Types[T] {
  ...
  switch (type) {
    ...
    case 'string[]':
      debugLog(inDebugMode, 'Handling type: string[]')
      if (Array.isArray(value)) {
        const result = value.map(String)
        debugLog(inDebugMode, `string[] case: value is array, returning`, JSON.stringify(result))
      ...
      if (value !== null && value !== undefined && typeof value !== 'boolean') {
        ...
        debugLog(inDebugMode, `string[] case: value is single, returning`, JSON.stringify(result))
        return result as Types[T]
      }
      debugLog(inDebugMode, 'string[] case: value is null/undefined/boolean, returning null')
    ...
    default:
      ...
      debugLog(inDebugMode, `Unhandled type encountered: ${type}`)
